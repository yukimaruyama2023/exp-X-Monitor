...
#         if not files_net:
#             print(f"[warn] No Netdata files for metric={metric}")
#         # util_netdata[mcd][interval] = total_pct (user: netdata+go.d, kernel: netdata)
#         util_netdata: Dict[int, Dict[str, float]] = {}
#         for mcd, by_interval in files_net.items():
#             util_netdata[mcd] = {}
#             for interval, fp in by_interval.items():
...
#                 if metric == "user":
#                     total = daemon + god      # ★ user のときは netdata + go.d.plugin
#                 else:
#                     total = daemon            # ★ kernel のときは netdata のみ

#                 util_netdata[mcd][interval] = total
...
# memcached_stats_loop (user metrics 用, Netdata 実験時)
MEMCACHED_STATS_LOOP_FNAME_RE = re.compile(
    r"memcached_stats_loop-usermetrics-(\d+)mcd-interval([0-9.]+)\.csv$"
)
...
# ---- ファイル探索（memcached_stats_loop, user metrics, Netdata 実験時）----
def _collect_memcached_statsloop_files(
    base_dir: Path,
    mcd_nums: Optional[List[int]] = None,
) -> Dict[int, Dict[str, Path]]:
    """
    戻り値: {mcd_num: {interval_str: Path, ...}}
    例: {12: {"1": Path(".../memcached_stats_loop-usermetrics-12mcd-interval1.csv"), ...}}
    """
    if mcd_nums is None:
        mcd_nums = [1, 5, 10]
    ...
    for mcd_num in mcd_nums:
        ...
        for p in sub.iterdir():
            if not p.is_file():
                continue
            m = MEMCACHED_STATS_LOOP_FNAME_RE.match(p.name)
            if not m:
                continue
            f_mcd, f_interval = m.group(1), m.group(2)
            if int(f_mcd) != mcd_num:
                continue
            if f_interval in intervals:
                available[f_interval] = p
        if available:
...
def make_plots(
    ...
    for metric in ("user", "kernel"):
        ...
        # memcached_stats_loop は user metrics の Netdata 実験のときだけ
        memcached_stats_files: Dict[int, Dict[str, Path]] = {}
        if metric == "user":
            memcached_stats_files = _collect_memcached_statsloop_files(base, mcd_nums)

        # util_netdata[mcd][interval] = total_pct
        util_netdata: Dict[int, Dict[str, float]] = {}
        # breakdown 用: go.d.plugin, memcached_stats (user metrics のときだけ使用)
        util_netdata_god: Dict[int, Dict[str, float]] = {}
        util_netdata_stats: Dict[int, Dict[str, float]] = {}

        for mcd, by_interval in files_net.items():
            util_netdata[mcd] = {}
            if metric == "user":
                util_netdata_god[mcd] = {}
                util_netdata_stats[mcd] = {}
            for interval, fp in by_interval.items():
                usage = parse_netdata_csv(fp)
                daemon = usage.get("netdata", np.nan)
                god = usage.get("go.d.plugin", 0.0)

                memcached_stats_cpu = 0.0
                if metric == "user":
                    # 対応する memcached_stats_loop ファイルがあれば読む
                    stats_fp = memcached_stats_files.get(mcd, {}).get(interval)
                    if stats_fp is not None:
                        stats_usage = parse_netdata_csv(stats_fp)
                        memcached_stats_cpu = stats_usage.get("memcached_stats", 0.0)

                if metric == "user":
                    total = daemon + god + memcached_stats_cpu
                else:
                    total = daemon
                ...
                if metric == "user":
                    util_netdata_god[mcd][interval] = god
                    util_netdata_stats[mcd][interval] = memcached_stats_cpu
        ...
        # 結果を保存
        if metric == "user":
            results[metric] = (util_xmon, util_netdata, util_netdata_god, util_netdata_stats)
        else:
