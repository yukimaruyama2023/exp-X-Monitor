...
#     "Netdata (500ms)":      "#F28F79",  # Peach Light
#     "Netdata (1ms)":        "#E46876",  # Peach Dark

#     # X-Monitor 系
#     "X-Monitor (1000ms)":   "#5AB8A8",  # Teal
#     "X-Monitor (500ms)":    "#72A7E3",  # Light Blue (旧 100ms 色)
#     "X-Monitor (1ms)":      "#C7A0E8",  # Lavender (旧 10ms 色)
# }
COLORS = {
    ...
    "Netdata (500ms)":      "#F28F79",  # Peach Light
    "Netdata (1ms)":        "#E46876",  # Peach Dark
    "Netdata (100μs)":      "#F28F79",  # ひとまず 1ms と同じ色

    # X-Monitor 系
    "X-Monitor (1000ms)":   "#5AB8A8",  # Teal
    "X-Monitor (500ms)":    "#72A7E3",  # Light Blue
    "X-Monitor (1ms)":      "#C7A0E8",  # Lavender
    "X-Monitor (100μs)":    "#72A7E3",  # ひとまず 1ms と同じ色
}
...
def interval_label(sec: float) -> str:
    """
    interval(sec) を ms または μs 表記で返す。
    - 1ms 以上  → ms 表記
    - 1ms 未満 → μs 表記
    """
    ms = sec * 1000.0
    if ms >= 1.0:
        ...
        return f"{ms:g}ms"
    # ---- ここから μs ----
    us = ms * 1000.0  # 1 ms = 1000 us
    # ほぼ整数なら整数表記
    if abs(us - round(us)) < 1e-6:
        return f"{int(round(us))}μs"
    return f"{us:g}μs"
...
def get_color_from_label(label: str):
    """
    ラベルそのものをキーとして COLORS から色を引く。
    例: "X-Monitor (1000ms)" / "Netdata (500ms)" など。
    """
...
def plot_one(ax, curves, xmax: float):
    ...
    ax.set_xticks(np.arange(0, xmax + 0.1, 1))
    ax.set_yticks([0.90, 0.99, 1.00], ["90%", "99%", "100%"])
    ax.set_xlabel("latency [ms]")
    ax.set_ylabel("percentile")
    ax.legend(loc='lower right', labelspacing=0, fontsize=legendsize)
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="save figures as PDF instead of showing"
    )
...
def main_impl(args) -> None:
    ...
    for sec, path in xmon_user:
        ...
        # ★ interval 1ms のときは先頭 120 サンプルだけ使う
        if abs(sec - 0.001) < 1e-9:
            lat = lat[:120]
    ...
    out_root = os.path.join(args.dir, "output-pdf", mode_dir)

    if args.save:
        os.makedirs(out_root, exist_ok=True)
        out1 = os.path.join(out_root, f"cdf-usermetrics-{mcd}mcd.pdf")
        out2 = os.path.join(out_root, f"cdf-kernelmetrics-{mcd}mcd.pdf")
        ...
        out1 = os.path.join(out_root, f"cdf-usermetrics-{mcd}mcd.pdf")
        out2 = os.path.join(out_root, f"cdf-kernelmetrics-{mcd}mcd.pdf")
        print("[Not saving files] (use --save to generate pdf)")
        print("  Would save as:")
        print(f"    {out1}")
        print(f"    {out2}\n")
