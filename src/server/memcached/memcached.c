...
enum try_read_result {
  ...
  READ_ERROR,       /** an error occurred (on the socket) (or client closed
                       connection) */
...
static void event_handler(const evutil_socket_t fd, const short which,
                          void *arg);
...
static void maxconns_handler(const evutil_socket_t fd, const short which,
                             void *arg) {
...
uint64_t get_physical_address(uint64_t virtual_address, pid_t pid) {
  ...
  uint64_t offset = (virtual_address / PAGE_SIZE) * PAGEMAP_ENTRY_SIZE;
  ...
  if (read(fd, &value, PAGEMAP_ENTRY_SIZE) != PAGEMAP_ENTRY_SIZE) {
    perror("read");
    close(fd);
    return -1;
  }
...
static inline unsigned long my_rdtsc(void) {
  unsigned int lo, hi;
  __asm__ __volatile__("mfence\n"
                       "lfence\n" /* see the description of RDTSC in Intel
                                     Software Developer's Manual Vol. 2B */
                       "rdtsc\n"
                       "lfence\n"
                       : "=a"(lo), "=d"(hi)
                       :
...
static void stats_init(void) {
  ...
  uint32_t n = 7;
  if (ioctl(fd, XMON_IOC_CREATE, &n) < 0) {
    perror("ioctl xmon_ioc_create ");
    return;
  }

  struct xmon_ioc_set_elem se_stats = {
      .idx = 0,
      .head_addr = (uint64_t)&stats,
      .size = 168,
  };

  struct xmon_ioc_set_elem se_stats_state = {
      .idx = 1,
      .head_addr = (uint64_t)&stats_state,
      .size = 56,
  };

  struct xmon_ioc_set_elem se_settings = {
      .idx = 2,
      .head_addr = (uint64_t)&settings,
      .size = 24,
  };

  struct xmon_ioc_set_elem se_rusage = {
      .idx = 3,
      .head_addr = (uint64_t)&rusage,
      .size = 32,
  };

  struct xmon_ioc_set_elem se_thread_stats = {
      .idx = 4,
      .head_addr = (uint64_t)&thread_stats,
      .size = 216,
  };

  struct xmon_ioc_set_elem se_slab_stats = {
      .idx = 5,
      .head_addr = (uint64_t)&slab_stats,
      .size = 64,
  };

  struct xmon_ioc_set_elem se_totals = {
      .idx = 6,
      .head_addr = (uint64_t)&totals,
      .size = 96,
  };

  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_stats) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_stats_state) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_settings) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_rusage) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_thread_stats) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_slab_stats) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  if (ioctl(fd, XMON_IOC_SET_ELEM, &se_totals) < 0) {
    perror("ioctl xmon_ioc_set_elem ");
    return;
  }
  ...
  // int id;
  // if ((id = syscall(465, 7)) < 0) {
  //   perror("Initialize descriptor array: ");
  //   return;
  // }
  //
  // /* xmon_register*/
  // if (syscall(466, id, 0, (uint64_t)&stats, 168) < 0) {
  //   perror("xmon_register: stats: ");
  //   return;
  // }
  // if (syscall(466, id, 1, (uint64_t)&stats_state, 56) < 0) {
  //   perror("xmon_register: stats_state: ");
  //   return;
  // }
  // if (syscall(466, id, 2, (uint64_t)&settings, 24) < 0) {
  //   perror("xmon_register: settings: ");
  //   return;
  // }
  // if (syscall(466, id, 3, (uint64_t)&rusage, 32) < 0) {
  //   perror("xmon_register: rusage: ");
  //   return;
  // }
  // if (syscall(466, id, 4, (uint64_t)&thread_stats, 216) < 0) {
  //   perror("xmon_register: thread_stats: ");
  //   return;
  // }
  // if (syscall(466, id, 5, (uint64_t)&slab_stats, 64) < 0) {
  //   perror("xmon_register: slab_stats: ");
  //   return;
  // }
  // if (syscall(466, id, 6, (uint64_t)&totals, 96) < 0) {
  //   perror("xmon_register: itemstats_t: ");
  //   return;
  // }
  //
  // /* register port number */
  // if (syscall(467, id, port_num) < 0) {
  ...
  printf("elapsed time for initialization is %ld\n", elapsed);
...
static void *conn_timeout_thread(void *arg) {
  ...
  while (do_run_conn_timeout_thread) {
    ...
    if (settings.verbose > 2)
      fprintf(stderr, "idle timeout thread finished pass, sleeping for %ds\n",
              sleep_time);
...
void thread_io_queue_add(LIBEVENT_THREAD *t, int type, void *ctx,
                         io_queue_stack_cb cb) {
...
void conn_io_queue_return(io_pending_t *io) { io->return_cb(io); }
...
void conn_release_items(conn *c) {
  ...
  if (c->resp_head) {
    ...
    while (resp) {
      ...
      if (resp->free) {
        fprintf(stderr,
                "ERROR: double free detected during conn_release_items(): [%d] "
                "[%s]\n",
...
static const char *state_text(enum conn_states state) {
  const char *const statenames[] = {
      ...
      "conn_closing",   "conn_mwrite",    "conn_closed",    "conn_watch",
      "conn_io_queue",  "conn_io_resume", "conn_io_pending"};
...
static int _store_item_copy_chunks(item *d_it, item *s_it, const int len) {
  ...
  if (s_it->it_flags & ITEM_CHUNKED) {
    ...
    while (len > done && dch) {
      int todo = (dch->size - dch->used < len - done) ? dch->size - dch->used
                                                      : len - done;
...
static int _store_item_copy_data(int comm, item *old_it, item *new_it,
                                 item *add_it) {
  if (comm == NREAD_APPEND || comm == NREAD_APPENDVIV) {
    if (new_it->it_flags & ITEM_CHUNKED) {
      ...
      memcpy(ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */,
             ITEM_data(add_it), add_it->nbytes);
    }
  } else {
    /* NREAD_PREPEND */
    if (new_it->it_flags & ITEM_CHUNKED) {
      ...
      memcpy(ITEM_data(new_it) + add_it->nbytes - 2 /* CRLF */,
 ...
 */
enum store_item_type do_store_item(item *it, int comm, LIBEVENT_THREAD *t,
                                   const uint32_t hv, int *nbytes,
                                   uint64_t *cas, uint64_t cas_in,
                                   bool cas_stale) {
  ...
  LOGGER_LOG(t->l, LOG_MUTATIONS, LOGGER_ITEM_STORE, NULL, stored, comm,
             ITEM_key(it), it->nkey, it->nbytes, it->exptime, ITEM_clsid(it),
             t->cur_sfd);
...
void server_stats(ADD_STAT add_stats, void *c) {
  ...
  APPEND_STAT("curr_connections", "%llu",
  ...
  APPEND_STAT("response_obj_oom", "%llu",
              (unsigned long long)thread_stats.response_obj_oom);
  APPEND_STAT("response_obj_count", "%llu",
              (unsigned long long)thread_stats.response_obj_count);
  APPEND_STAT("response_obj_bytes", "%llu",
              (unsigned long long)thread_stats.response_obj_bytes);
  APPEND_STAT("read_buf_count", "%llu",
              (unsigned long long)thread_stats.read_buf_count);
  APPEND_STAT("read_buf_bytes", "%llu",
              (unsigned long long)thread_stats.read_buf_bytes);
  APPEND_STAT("read_buf_bytes_free", "%llu",
              (unsigned long long)thread_stats.read_buf_bytes_free);
  ...
  if (settings.proxy_enabled) {
    ...
    APPEND_STAT("proxy_conn_errors", "%llu",
                (unsigned long long)thread_stats.proxy_conn_errors);
    APPEND_STAT("proxy_conn_oom", "%llu",
                (unsigned long long)thread_stats.proxy_conn_oom);
    APPEND_STAT("proxy_req_active", "%llu",
                (unsigned long long)thread_stats.proxy_req_active);
  }
#endif
  ...
  if (ext_storage) { // false
    ...
    APPEND_STAT("get_aborted_extstore", "%llu",
                (unsigned long long)thread_stats.get_aborted_extstore);
    APPEND_STAT("get_oom_extstore", "%llu",
                (unsigned long long)thread_stats.get_oom_extstore);
    APPEND_STAT("recache_from_extstore", "%llu",
                (unsigned long long)thread_stats.recache_from_extstore);
    APPEND_STAT("miss_from_extstore", "%llu",
                (unsigned long long)thread_stats.miss_from_extstore);
    APPEND_STAT("badcrc_from_extstore", "%llu",
                (unsigned long long)thread_stats.badcrc_from_extstore);
  }
#endif
  ...
  APPEND_STAT("store_too_large", "%llu",
              (unsigned long long)thread_stats.store_too_large);
  APPEND_STAT("store_no_memory", "%llu",
              (unsigned long long)thread_stats.store_no_memory);
  ...
  APPEND_STAT("time_in_listen_disabled_us", "%llu",
  ...
  if (settings.slab_reassign) { // true
    ...
    APPEND_STAT("slab_reassign_evictions_nomem", "%llu",
  ...
  APPEND_STAT("log_worker_dropped", "%llu",
              (unsigned long long)stats.log_worker_dropped);
  APPEND_STAT("log_worker_written", "%llu",
              (unsigned long long)stats.log_worker_written);
  APPEND_STAT("log_watcher_skipped", "%llu",
              (unsigned long long)stats.log_watcher_skipped);
  APPEND_STAT("log_watcher_sent", "%llu",
              (unsigned long long)stats.log_watcher_sent);
  APPEND_STAT("log_watchers", "%llu",
              (unsigned long long)stats_state.log_watchers);
  STATS_UNLOCK();
#ifdef EXTSTORE
  ...
  if (settings.ssl_enabled) {
    if (settings.ssl_session_cache) {
      APPEND_STAT("ssl_new_sessions", "%llu",
                  (unsigned long long)stats.ssl_new_sessions);
    }
    APPEND_STAT("ssl_handshake_errors", "%llu",
                (unsigned long long)stats.ssl_handshake_errors);
    APPEND_STAT("ssl_proto_errors", "%llu",
                (unsigned long long)stats.ssl_proto_errors);
    APPEND_STAT("time_since_server_cert_refresh", "%u",
                now - settings.ssl_last_cert_refresh_time);
  }
#endif
  APPEND_STAT("unexpected_napi_ids", "%llu",
              (unsigned long long)stats.unexpected_napi_ids);
  APPEND_STAT("round_robin_fallback", "%llu",
              (unsigned long long)stats.round_robin_fallback);
}
...
void process_stat_settings(ADD_STAT add_stats, void *c) {
  ...
  APPEND_STAT("auth_enabled_ascii", "%s",
              settings.auth_file ? settings.auth_file : "no");
  ...
  APPEND_STAT("lru_crawler_tocrawl", "%lu",
              (unsigned long)settings.lru_crawler_tocrawl);
  ...
  APPEND_STAT("lru_maintainer_thread", "%s",
              settings.lru_maintainer_thread ? "yes" : "no");
  ...
  APPEND_STAT("inline_ascii_response", "%s",
              "no"); // setting is dead, cannot be yes.
  ...
  APPEND_STAT("slab_automove_freeratio", "%.3f",
              settings.slab_automove_freeratio);
  ...
  APPEND_STAT("ssl_ciphers", "%s",
              settings.ssl_ciphers ? settings.ssl_ciphers : "NULL");
  APPEND_STAT("ssl_ca_cert", "%s",
              settings.ssl_ca_cert ? settings.ssl_ca_cert : "NULL");
  APPEND_STAT("ssl_wbuf_size", "%u", settings.ssl_wbuf_size);
  APPEND_STAT("ssl_session_cache", "%s",
              settings.ssl_session_cache ? "yes" : "no");
  APPEND_STAT("ssl_kernel_tls", "%s", settings.ssl_kernel_tls ? "yes" : "no");
  APPEND_STAT("ssl_min_version", "%s",
...
static inline void get_conn_text(const conn *c, const int af, char *addr,
  ...
  switch (af) {
  case AF_INET:
    (void)inet_ntop(af, &((struct sockaddr_in *)sock_addr)->sin_addr, addr_text,
                    sizeof(addr_text) - 1);
  ...
  case AF_INET6:
    addr_text[0] = '[';
    addr_text[1] = '\0';
    if (inet_ntop(af, &((struct sockaddr_in6 *)sock_addr)->sin6_addr,
                  addr_text + 1, sizeof(addr_text) - 2)) {
      strncat(addr_text, "]", 2);
    }
  ...
  case AF_UNIX: {
    ...
    strncpy(addr_text, ((struct sockaddr_un *)sock_addr)->sun_path, pathlen);
    addr_text[pathlen] = '\0';
    protoname = "unix";
  } break;
...
static void conn_to_str(const conn *c, char *addr, char *svr_addr) {
  if (!c) {
    ...
    if (c->state == conn_listening ||
        (IS_UDP(c->transport) && c->state == conn_read)) {
      memset(&local_addr, 0, sizeof(local_addr));
      socklen_t local_addr_len = sizeof(local_addr);

      if (getsockname(c->sfd, (struct sockaddr *)&local_addr,
                      &local_addr_len) == 0) {
    ...
    if (c->state != conn_listening &&
      ...
      get_conn_text(c, svr_sock_addr.ss_family, svr_addr,
                    (struct sockaddr *)&svr_sock_addr);
    }
  }
}
  ...
  for (i = 0; i < max_fds; i++) {
    if (conns[i]) {
      ...
      if (conns[i]->state != conn_closed) {
        ...
        APPEND_NUM_STAT(i, "state", "%s", state_text(sc->state));
...
item *limited_get(const char *key, size_t nkey, LIBEVENT_THREAD *t,
                  uint32_t exptime, bool should_touch, bool do_update,
                  bool *overflow) {
  item *it;
  if (should_touch) {
...
// items.c.
item *limited_get_locked(const char *key, size_t nkey, LIBEVENT_THREAD *t,
                         bool do_update, uint32_t *hv, bool *overflow) {
  item *it;
...
static int _transmit_pre(conn *c, struct iovec *iovs, int iovused,
  ...
  while (resp && iovused + resp->iovcnt < IOV_MAX - 1) {
    ...
    if (resp->chunked_data_iov) {
      ...
      item_chunk *ch = (item_chunk *)ITEM_schunk(
          (item *)resp->iov[resp->chunked_data_iov].iov_base);
      int x;
      for (x = 0; x < resp->iovcnt; x++) {
        ...
        if (x == resp->chunked_data_iov) {
          ...
          while (ch && todo > 0 && iovused < IOV_MAX - 1) {
            ...
            iovs[iovused].iov_len =
                ch->used - skip > todo ? todo : ch->used - skip;
            iovused++;
            todo -= ch->used - skip;
            ch = ch->next;
          }
  ...
  while (c->rlbytes > 0) {
    ...
    if (ch->size == ch->used) {
      ...
      if (ch->next) {
        ...
        c->ritem = (char *)do_item_alloc_chunk(
            ch, c->rlbytes + ((c->protocol == binary_prot) ? 2 : 0));
        if (!c->ritem) {
...
static void drive_machine(conn *c) {
  ...
  while (!stop) {
    ...
    case conn_listening:
      ...
      if (use_accept4) {
        sfd =
            accept4(c->sfd, (struct sockaddr *)&addr, &addrlen, SOCK_NONBLOCK);
      } else {
    ...
    case conn_nread:
      ...
      if (settings.verbose > 0) {
        fprintf(stderr,
                ...
                errno, strerror(errno), (void *)c->rcurr, (void *)c->ritem,
                (void *)c->rbuf, (int)c->rlbytes, (int)c->rsize);
      }
      conn_set_state(c, conn_closing);
      break;
...
#ifdef _WIN32
  if (getsockopt((SOCKET)sfd, SOL_SOCKET, SO_SNDBUF, (char *)&old_size,
                 &intsize) != 0) {
#else
...
static int server_socket(const char *interface, int port,
  ...
  for (next = ai; next; next = next->ai_next) {
    ...
    if (settings.sock_cookie_id != 0) {
      error = setsockopt(sfd, SOL_SOCKET, SOCK_COOKIE_ID,
                         (void *)&settings.sock_cookie_id, sizeof(uint32_t));
      if (error != 0)
        perror("setsockopt");
    }
#endif
    ...
    if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
      ...
      if (portnumber_file != NULL && (next->ai_addr->sa_family == AF_INET ||
    ...
    if (IS_UDP(transport)) {
      ...
      for (c = 0; c < settings.num_threads_per_udp; c++) {
        ...
        dispatch_conn_new(per_thread_fd, conn_read, EV_READ | EV_PERSIST,
...
static int server_sockets(int port, enum network_transport transport,
  ...
  if (settings.inter == NULL) {
    return server_socket(settings.inter, port, transport, portnumber_file,
                         ssl_enabled, 0, settings.binding_protocol);
  } else {
    ...
    for (char *p = strtok_r(list, ";,", &b); p != NULL;
        ...
        if (*p == '[' || *p == '_') {
          ...
          *e = '\0';
          ...
          if (len > 8 || len < 1) {
            fprintf(
                stderr,
                "Listener tags must be between 1 and 8 characters: \"%s\"\n",
                st);
            free(list);
            return 1;
          }
      ...
      if (s != NULL) {
        ...
        // until of course there was an RFC3986 IPv6 address previously
        // specified - in such a case there is no good option, will just send it
        // to fail as port number.
...
static void clock_handler(const evutil_socket_t fd, const short which,
                          void *arg) {
...
static void usage(void) {
...
#ifndef DISABLE_UNIX_SOCKET
  printf("-s, --unix-socket=<file>  UNIX socket to listen on (disables network "
         "support)\n");
  printf("-a, --unix-mask=<mask>    access mask for UNIX socket, in octal "
         "(default: %o)\n",
         settings.access);
#endif /* #ifndef DISABLE_UNIX_SOCKET */
  printf("-A, --enable-shutdown     enable ascii \"shutdown\" command\n");
  printf("-l, --listen=<addr>       interface to listen on (default: "
         "INADDR_ANY)\n");
#ifdef TLS
  printf("                          if TLS/SSL is enabled, 'notls' prefix can "
         "be used to\n"
  ...
  printf(
      ...
      "-u, --user=<user>         assume identity of <username> (only when run "
      "as root)\n"
      "-m, --memory-limit=<num>  item memory in megabytes (default: %lu)\n"
      "-M, --disable-evictions   return error on memory exhausted instead of "
      "evicting\n"
      "-c, --conn-limit=<num>    max simultaneous connections (default: %d)\n"
      "-k, --lock-memory         lock down all paged memory\n"
      "-v, --verbose             verbose (print errors/warnings while in event "
      "loop)\n"
      "-vv                       very verbose (also print client "
      "commands/responses)\n"
      ...
      "-f, --slab-growth-factor=<num> chunk size growth factor (default: "
      "%2.2f)\n"
      "-n, --slab-min-size=<bytes> min space used for key+value+flags "
      "(default: %d)\n",
      (unsigned long)settings.maxbytes / (1 << 20), settings.maxconns,
      settings.factor, settings.chunk_size);
  verify_default("udp-port", settings.udpport == 0);
  printf("-L, --enable-largepages  try to use large memory pages (if "
         "available)\n");
  printf("-D <char>     Use <char> as the delimiter between key prefixes and "
         "IDs.\n"
         "              This is used for per-prefix stats reporting. The "
         "default is\n"
         "              \"%c\" (colon). If this option is specified, stats "
         "collection\n"
         "              is turned on automatically; if not, then it may be "
         "turned on\n"
         "              by sending the \"stats detail on\" command to the "
         "server.\n",
         settings.prefix_delimiter);
  printf("-t, --threads=<num>       number of threads to use (default: %d)\n",
         settings.num_threads);
  printf("-R, --max-reqs-per-event  maximum number of requests per event, "
         "limits the\n"
         "                          requests processed per connection to "
         "prevent \n"
         "                          starvation (default: %d)\n",
         settings.reqs_per_event);
  printf("-C, --disable-cas         disable use of CAS\n");
  printf(
      "-b, --listen-backlog=<num> set the backlog queue limit (default: %d)\n",
      settings.backlog);
  printf("-B, --protocol=<name>     protocol - one of ascii, binary, or auto "
         "(default: %s)\n",
         prot_text(settings.binding_protocol));
  printf("-I, --max-item-size=<num> adjusts max item size\n"
         "                          (default: %dm, min: %dk, max: %dm)\n",
         settings.item_size_max / (1 << 20),
         ITEM_SIZE_MAX_LOWER_LIMIT / (1 << 10),
         ITEM_SIZE_MAX_UPPER_LIMIT / (1 << 20));
#ifdef ENABLE_SASL
  printf("-S, --enable-sasl         turn on Sasl authentication\n");
#endif
  printf("-F, --disable-flush-all   disable flush_all command\n");
  printf("-X, --disable-dumping     disable stats cachedump and lru_crawler "
         "metadump\n");
  printf("-W  --disable-watch       disable watch commands (live logging)\n");
  printf("-Y, --auth-file=<file>    (EXPERIMENTAL) enable ASCII protocol "
         "authentication. format:\n"
         "                          user:pass\\nuser2:pass2\\n\n");
  printf(
      "-e, --memory-file=<file>  (EXPERIMENTAL) mmap a file for item memory.\n"
      "                          use only in ram disks or persistent memory "
      "mounts!\n"
      "                          enables restartable cache (stop with "
      "SIGUSR1)\n");
#ifdef TLS
  ...
  printf(
      ...
      "                          default is murmur3 hash. options: jenkins, "
      "murmur3, xxh3\n"
  ...
  printf("   - read_buf_mem_limit:  limit in megabytes for connection "
         "read/response buffers.\n"
         "                          do not adjust unless you have high (20k+) "
         "conn. limits.\n"
  ...
  printf(
      ...
      "                          (requires lru_maintainer, default pct: %d)\n"
      "   - hot_max_factor:      items idle > cold lru age * drop from hot "
      "lru. (default: %.2f)\n"
      "   - warm_max_factor:     items idle > cold lru age * this drop from "
      "warm. (default: %.2f)\n"
      "   - temporary_ttl:       TTL's below get separate LRU, can't be "
      "evicted.\n"
      "                          (requires lru_maintainer, default: %d)\n"
      "   - idle_timeout:        timeout for idle connections. (default: %d, "
      "no timeout)\n",
      settings.hot_lru_pct, settings.warm_lru_pct, settings.hot_max_factor,
      settings.warm_max_factor, settings.temporary_ttl, settings.idle_timeout);
  printf("   - slab_chunk_max:      (EXPERIMENTAL) maximum slab size in "
         "kilobytes. use extreme care. (default: %d)\n"
         "   - watcher_logbuf_size: size in kilobytes of per-watcher write "
         "buffer. (default: %u)\n"
         "   - worker_logbuf_size:  size in kilobytes of per-worker-thread "
         "buffer\n"
         "                          read by background thread, then written to "
         "watchers. (default: %u)\n"
         "   - track_sizes:         enable dynamic reports for 'stats sizes' "
         "command.\n"
         "                          note that counts for each size are "
         "approximate.\n"
         "   - no_hashexpand:       disables hash table expansion (dangerous)\n"
         "   - modern:              enables options which will be default in "
         "future.\n"
         "                          currently: nothing\n"
         "   - no_modern:           uses defaults of previous major version "
         "(1.4.x)\n",
         settings.slab_chunk_size_max / (1 << 10),
         settings.logger_watcher_buf_size / (1 << 10),
         settings.logger_buf_size / (1 << 10));
  verify_default("tail_repair_time",
                 settings.tail_repair_time == TAIL_REPAIR_TIME_DEFAULT);
  ...
  printf("   - drop_privileges:     enable dropping extra syscall privileges\n"
         "   - no_drop_privileges:  disable drop_privileges in case it causes "
         "issues with\n"
         "                          some customisation.\n"
         "                          (default is no_drop_privileges)\n");
  verify_default("drop_privileges", !settings.drop_privileges);
#ifdef MEMCACHED_DEBUG
  printf("   - relaxed_privileges:  running tests requires extra privileges. "
         "(default: %s)\n",
         flag_enabled_disabled(settings.relaxed_privileges));
#endif
#endif
#ifdef SOCK_COOKIE_ID
  printf("   - sock_cookie_id:      attributes an ID to a socket for ip "
         "filtering/firewalls \n");
#endif
#ifdef EXTSTORE
  printf("\n   - External storage (ext_*) related options (see: "
         "https://memcached.org/extstore)\n");
  printf(
      "   - ext_path:            file to write to for external storage.\n"
      "                          ie: ext_path=/mnt/d1/extstore:1G\n"
      "   - ext_page_size:       size in megabytes of storage pages. (default: "
      "%u)\n"
      "   - ext_wbuf_size:       size in megabytes of page write buffers. "
      "(default: %u)\n"
      "   - ext_threads:         number of IO threads to run. (default: %u)\n"
      "   - ext_item_size:       store items larger than this (bytes, default "
      "%u)\n"
      "   - ext_item_age:        store items idle at least this long (seconds, "
      "default: no age limit)\n"
      "   - ext_low_ttl:         consider TTLs lower than this specially "
      "(default: %u)\n"
      "   - ext_drop_unread:     don't re-write unread values during "
      "compaction (default: %s)\n"
      "   - ext_recache_rate:    recache an item every N accesses (default: "
      "%u)\n"
      "   - ext_compact_under:   compact when fewer than this many free pages\n"
      "                          (default: 1 percent of the assigned storage)\n"
      "   - ext_drop_under:      drop COLD items when fewer than this many "
      "free pages\n"
      "                          (default: 1/4th of the assigned storage)\n"
      "   - ext_max_frag:        only defrag pages if they are less full than "
      "this pct-wise (default: %.2f)\n"
      "   - ext_max_sleep:       max sleep time of background threads in us "
      "(default: %u)\n"
      "   - slab_automove_freeratio: ratio of memory to hold free as buffer.\n"
      "                          (see doc/storage.txt for more info, default: "
      "%.3f)\n",
      settings.ext_page_size / (1 << 20), settings.ext_wbuf_size / (1 << 20),
      settings.ext_io_threadcount, settings.ext_item_size, settings.ext_low_ttl,
      flag_enabled_disabled(settings.ext_drop_unread),
      settings.ext_recache_rate, settings.ext_max_frag, settings.ext_max_sleep,
      settings.slab_automove_freeratio);
  verify_default("ext_item_age", settings.ext_item_age == UINT_MAX);
#endif
#ifdef PROXY
  printf("   - proxy_config:        path to lua library file. separate with "
         "':' for multiple files\n");
  printf("   - proxy_arg:           string to pass to lua library\n");
#endif
  ssl_help();
  printf("-N, --napi_ids            number of napi ids. see doc/napi_ids.txt "
         "for more details\n");
  return;
}
...
static void save_pid(const char *pid_file) {
    ...
    if ((fp = fopen(pid_file, "r")) != NULL) {
      ...
      if (fgets(buffer, sizeof(buffer), fp) != NULL) {
        ...
        if (safe_strtoul(buffer, &pid) && kill((pid_t)pid, 0) == 0) {
          fprintf(stderr,
                  "WARNING: The pid file contained the following (running) "
                  "pid: %u\n",
                  pid);
        }
      }
  ...
  if (rename(tmp_pid_file, pid_file) != 0) {
    vperror("Could not rename the pid file from %s to %s", tmp_pid_file,
            pid_file);
  }
}
...
static void sig_handler(const int sig) { stop_main_loop = EXIT_NORMALLY; }

static void sighup_handler(const int sig) { settings.sig_hup = true; }

static void sig_usrhandler(const int sig) { stop_main_loop = GRACE_STOP; }
...
static int enable_large_pages(void) {
  ...
  static const char *mm_thp_paths[] = {
      ...
      "/sys/kernel/mm/redhat_transparent_hugepage/enabled", NULL};
  ...
  if (rd <= 0) {
    fprintf(stderr,
            "Transparent huge pages could not read the configuration.\n");
    fprintf(stderr, "Will use default page size.\n");
    return -1;
  }
  thpb[rd] = 0;
  ...
  if (sysctlbyname("vm.pmap.pg_ps_enabled", &spages, &spagesl, NULL, 0) != 0) {
...
static bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
  ...
  for (char *p = strtok_r(s, "-", &b); p != NULL; p = strtok_r(NULL, "-", &b)) {
    ...
    if (last_size >= size) {
      fprintf(stderr,
              "slab size %u cannot be lower than or equal to a previous class "
              "size\n",
              size);
      return false;
    }
    if (size <= last_size + CHUNK_ALIGN_BYTES) {
      fprintf(
          stderr,
          "slab size %u must be at least %d bytes larger than previous class\n",
...
// Time is the hardest; we have to assume the sys clock is correct and re-sync
// for the lost time after restart.
static int _mc_meta_save_cb(const char *tag, void *ctx, void *data) {
  ...
  restart_set_kv(ctx, "maxbytes", "%llu",
  ...
  restart_set_kv(ctx, "slab_chunk_size_max", "%d",
  ...
  restart_set_kv(ctx, "slab_reassign", "%s",
                 settings.slab_reassign ? "true" : "false");
  ...
  restart_set_kv(ctx, "process_started", "%llu",
                 (unsigned long long)process_started);
  {
    struct timeval tv;
    gettimeofday(&tv, NULL);
...
// TODO: Once crc32'ing of the metadata file is done this could be ensured
// better by the restart module itself (crc32 + count of lines must match on the
// backend)
...
static int _mc_meta_load_cb(const char *tag, void *ctx, void *data) {
  ...
  const char *opts[] = {[R_MMAP_OLDBASE] = "mmap_oldbase",
                        [R_MAXBYTES] = "maxbytes",
  ...
  while (restart_get_kv(ctx, &key, &val) == RESTART_OK) {
    ...
    case R_SLAB_CHUNK_SIZE_MAX:
      if (!safe_strtol(val, &val_int) ||
          settings.slab_chunk_size_max != val_int) {
        reuse_mmap = -1;
      }
      break;
    ...
    if (reuse_mmap != 0) {
      fprintf(stderr,
              "[restart] restart incompatible due to setting for [%s] [old "
              "value: %s]\n",
              key, val);
      break;
    }
...
int main(int argc, char **argv) {
  ...
  char *shortopts = "a:"  /* access mask for unix socket */
  ...
  while (-1 != (c = getopt(argc, argv, shortopts))) {
    ...
    case 'o': /* It's sub-opts time! */
      subopts_orig = subopts =
          strdup(optarg); /* getsubopt() changes the original args */
      ...
      while (*subopts != '\0') {
        ...
        case HASHPOWER_INIT:
          ...
          if (settings.hashpower_init < 12) {
            ...
            fprintf(stderr,
                    ...
                    "Choose a value based on \"STAT hash_power_level\" from a "
                    "running instance\n",
                    settings.hashpower_init);
            return 1;
          }
        ...
        case SLAB_AUTOMOVE_RATIO:
          ...
          if (settings.slab_automove_ratio <= 0 ||
              settings.slab_automove_ratio > 1) {
        ...
        case TAIL_REPAIR_TIME:
          ...
          if (settings.tail_repair_time < 10) {
            fprintf(stderr,
                    "Cannot set tail_repair_time to less than 10 seconds\n");
            return 1;
          }
          break;
        case HASH_ALGORITHM:
          ...
          if (strcmp(subopts_value, "jenkins") == 0) {
            ...
            fprintf(stderr,
                    "Unknown hash_algorithm option (jenkins, murmur3, xxh3)\n");
            return 1;
          }
          break;
        case LRU_CRAWLER:
        ...
        case LRU_CRAWLER_SLEEP:
          ...
          if (settings.lru_crawler_sleep > 1000000 ||
              settings.lru_crawler_sleep < 0) {
            fprintf(stderr,
                    "LRU crawler sleep must be between 0 and 1 second\n");
            return 1;
          }
          break;
        case LRU_CRAWLER_TOCRAWL:
          ...
          if (!safe_strtoul(subopts_value, &tocrawl)) {
            fprintf(stderr,
                    "lru_crawler_tocrawl takes a numeric 32bit value\n");
            return 1;
          }
        ...
        case WATCHER_LOGBUF_SIZE:
          ...
          if (!safe_strtoul(subopts_value, &settings.logger_watcher_buf_size)) {
            fprintf(stderr,
                    "could not parse argument to watcher_logbuf_size\n");
            return 1;
          }
        ...
        case SSL_VERIFY_MODE: {
          ...
          if (!ssl_set_verify_mode(verify)) {
            fprintf(
                stderr,
                "Invalid ssl_verify_mode. Use help to see valid options.\n");
            return 1;
          }
          break;
        }
        ...
        case SSL_MIN_VERSION: {
          ...
          if (!ssl_set_min_version(min_version)) {
            fprintf(
                stderr,
                "Invalid ssl_min_version. Use help to see valid options.\n");
            return 1;
          }
          break;
        }
        ...
        case RESP_OBJ_MEM_LIMIT:
          ...
          fprintf(stderr, "DEPRECATED: resp_obj_mem_limit no longer used. See "
                          "read_buf_mem_limit,\n");
          break;
        ...
        case PROXY_CONFIG:
          ...
          if (protocol_specified) {
            fprintf(stderr,
                    "Cannot specify a protocol with proxy mode enabled\n");
            return 1;
          }
  ...
  if (settings.num_napi_ids > settings.num_threads) {
    fprintf(
        stderr,
        "Number of napi_ids(%d) cannot be greater than number of threads(%d)\n",
  ...
  if (settings.slab_chunk_size_max > settings.item_size_max) {
    fprintf(stderr,
            "slab_chunk_max (bytes: %d) cannot be larger than -I "
            "(item_size_max %d)\n",
  ...
  if (settings.item_size_max % settings.slab_chunk_size_max != 0) {
    fprintf(stderr,
            "-I (item_size_max: %d) must be evenly divisible by slab_chunk_max "
            "(bytes: %d)\n",
  ...
  if (settings.slab_page_size % settings.slab_chunk_size_max != 0) {
    fprintf(stderr,
            "slab_chunk_max (bytes: %d) must divide evenly into %d "
            "(slab_page_size)\n",
            settings.slab_chunk_size_max, settings.slab_page_size);
    exit(EX_USAGE);
  }
  ...
  if (settings.hot_lru_pct + settings.warm_lru_pct > 80) {
    fprintf(stderr,
            "hot_lru_pct + warm_lru_pct cannot be more than 80%% combined\n");
    exit(EX_USAGE);
  }
  ...
  if (settings.auth_file) {
    if (!protocol_specified) {
      ...
      if (settings.binding_protocol != ascii_prot) {
        fprintf(stderr, "ERROR: You cannot allow the BINARY protocol while "
                        "using ascii authentication tokens.\n");
        exit(EX_USAGE);
      }
    }
  }
  ...
  if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
      ...
      fprintf(stderr, "failed to set rlimit for open files. Try starting as "
...
#else
    fprintf(stderr, "warning: -k invalid, mlockall() not supported on this "
  ...
  if (settings.auth_file) {
    if (settings.udpport) {
      fprintf(stderr, "Cannot use UDP with ascii authentication enabled (-U 0 "
                      "to disable)\n");
      exit(EX_USAGE);
    }
    ...
    case AUTHFILE_OPENFAIL:
      vperror("Could not open authfile [%s] for reading, error %s",
              settings.auth_file, strerror(errno));
      exit(EXIT_FAILURE);
      break;
    case AUTHFILE_OOM:
      fprintf(stderr, "Out of memory reading password file: %s",
              settings.auth_file);
      exit(EXIT_FAILURE);
      break;
    case AUTHFILE_MALFORMED:
      fprintf(stderr,
              "Authfile [%s] has a malformed entry. Should be 'user:password'",
              settings.auth_file);
      exit(EXIT_FAILURE);
      break;
    case AUTHFILE_OK:
  ...
  if (settings.memory_file != NULL) {
    ...
    reuse_mem =
        restart_mmap_open(settings.maxbytes, settings.memory_file, &mem_base);
  ...
  if (storage_enabled && reuse_mem) {
    fprintf(
        stderr,
        "[restart] memory restart with extstore not presently supported.\n");
    reuse_mem = false;
  }
#endif
  ...
  if (settings.proxy_enabled) {
    settings.proxy_ctx =
        proxy_init(settings.proxy_uring, settings.proxy_memprofile);
  }
#endif
#ifdef EXTSTORE
  ...
  if (settings.socketpath == NULL) {
    ...
    if (portnumber_filename != NULL) {
      ...
      snprintf(temp_portnumber_filename, len, "%s.lck", portnumber_filename);
