 ...
 *
 * Portions of this file are available under BSD3 terms; see REDISCONTRIBUTIONS
 ...
 *
 * Since robj is being overused as general purpose object, `kvobj` distincts
 * only at the declarative level. This distinction assist to the clarity of the
 * code and can optionally enforce explicit casting later on. An `robj` is
 * identified to be `kvobj` if `iskvobj` flag is set.
 *
...
#define OBJ_SHARED_HDR_STRLEN(_len_)                                           \
  (((_len_) < 10) ? 4 : 5) /* see shared.mbulkhdr etc. */
...
#define NET_ADDR_STR_LEN (NET_IP_STR_LEN + 32) /* Must be enough for ip:port   \
                                                */
#define NET_HOST_PORT_STR_LEN                                                  \
  (NET_HOST_STR_LEN + 32) /* Must be enough for hostname:port */
...
#define CLIENT_MEM_USAGE_BUCKET_MIN_LOG                                        \
  15 /* Bucket sizes start at up to 32KB (2^15) */
#define CLIENT_MEM_USAGE_BUCKET_MAX_LOG                                        \
  33 /* Bucket for largest clients: sizes above 4GB (2^32) */
#define CLIENT_MEM_USAGE_BUCKETS                                               \
  (1 + CLIENT_MEM_USAGE_BUCKET_MAX_LOG - CLIENT_MEM_USAGE_BUCKET_MIN_LOG)
...
#define STATS_METRIC_NET_INPUT_REPLICATION                                     \
  3 /* Bytes read to network during replication. */
#define STATS_METRIC_NET_OUTPUT_REPLICATION                                    \
  4 /* Bytes written to network during replication. */
...
#define PROTO_RESIZE_THRESHOLD                                                 \
  (1024 * 32) /* Threshold for determining whether to resize query buffer */
...
/* Command flags. Please check the definition of struct redisCommand in this
 * file for more information about the meaning of every flag. */
...
#define CMD_MODULE_GETCHANNELS                                                 \
  (1ULL << 27) /* Use the modules getchannels interface. */
...
#define CMD_KEY_RW                                                             \
  (1ULL << 1) /* Read-Write - Reads and modifies/deletes                       \
...
#define CMD_KEY_RM (1ULL << 3) /* Deletes the key without reading it's value.  \
                                */
/* The following refer to user data inside the value of the key, not the
 * metadata like LRU, type, cardinality. It refers to the logical operation on
 * the user's data (actual input strings / TTL), being used / returned / copied
 * / changed, It doesn't refer to modification or returning of metadata (like
 * type, count, presence of data). Any write that's not INSERT or DELETE, would
 * be an UPDATE. Each key-spec may have one of the writes with or without
 * access, or none: */
...
#define CMD_KEY_PREFIX                                                         \
  (1ULL << 11) /* Given key represents a prefix of a set of keys */
...
#define DB_FLAG_NO_UPDATE_KEYSIZES                                             \
  (1ULL << 4) /* Don't update keysizes histograms */
...
#define CMD_CHANNEL_PATTERN (1ULL << 11) /* The argument is a channel pattern  \
                                          */
#define CMD_CHANNEL_SUBSCRIBE                                                  \
  (1ULL << 12) /* The command subscribes to channels */
#define CMD_CHANNEL_UNSUBSCRIBE                                                \
  (1ULL << 13) /* The command unsubscribes to channels */
...
#define CLIENT_EXECUTING_COMMAND                                               \
  (1 << 29) /* Indicates that the client is currently in the process of        \
             handling a command. usually this will be marked only during                            \
             call() however, blocked clients might have this flag kept until                      \
             they will try to reprocess the command. */
...
#define CLIENT_NO_TOUCH                                                        \
  (1ULL << 45) /* This client will not touch LFU/LRU stats. */
...
#define CLIENT_MODULE_PREVENT_AOF_PROP                                         \
  (1ULL << 48) /* Module client do not want to propagate to AOF */
#define CLIENT_MODULE_PREVENT_REPL_PROP                                        \
  (1ULL << 49) /* Module client do not want to propagate to replica */
#define CLIENT_REEXECUTING_COMMAND                                             \
  (1ULL << 50) /* The client is re-executing the command. */
#define CLIENT_REPL_RDB_CHANNEL                                                \
  (1ULL << 51) /* Client which is used for rdb delivery as part of rdb channel \
                  replication */
#define CLIENT_INTERNAL (1ULL << 52) /* Internal client connection */
#define CLIENT_ASM_MIGRATING                                                   \
  (1ULL << 53) /* Client is migrating RDB/stream data during atomic slot       \
                  migration. */
#define CLIENT_ASM_IMPORTING                                                   \
  (1ULL << 54) /* Client is importing RDB/stream data during atomic slot       \
                  migration. */

/* Any flag that does not let optimize FLUSH SYNC to run it in bg as blocking
 * client ASYNC */
#define CLIENT_AVOID_BLOCKING_ASYNC_FLUSH                                      \
  (CLIENT_DENY_BLOCKING | CLIENT_MULTI | CLIENT_LUA_DEBUG |                    \
   CLIENT_LUA_DEBUG_SYNC | CLIENT_MODULE)
...
#define CLIENT_IO_PENDING_COMMAND                                              \
  (1ULL << 2) /* Similar to CLIENT_PENDING_COMMAND. */
#define CLIENT_IO_REUSABLE_QUERYBUFFER                                         \
  (1ULL << 3) /* The client is using the reusable query buffer. */
#define CLIENT_IO_CLOSE_ASAP                                                   \
  (1ULL << 4) /* Close this client ASAP in IO thread. */
#define CLIENT_IO_PENDING_CRON                                                 \
  (1ULL                                                                        \
   << 5) /* The client is pending cron job, to be processed in main thread. */
...
#define SLAVE_STATE_WAIT_RDB_CHANNEL                                           \
  11 /* Main channel of replica is connected,                                  \
      * we are waiting rdbchannel connection to start delivery.*/
#define SLAVE_STATE_SEND_BULK_AND_STREAM                                       \
  12 /* Main channel of a replica which uses rdb channel replication.          \
...
#define SLAVE_CAPA_RDB_CHANNEL_REPL                                            \
  (1 << 2) /* Supports rdb channel replication during full sync */

/* Slave requirements */
#define SLAVE_REQ_NONE 0
#define SLAVE_REQ_RDB_EXCLUDE_DATA (1 << 0) /* Exclude data from RDB */
#define SLAVE_REQ_RDB_EXCLUDE_FUNCTIONS                                        \
  (1 << 1)                                /* Exclude functions from RDB */
#define SLAVE_REQ_SLOTS_SNAPSHOT (1 << 2) /* Only slots snapshot is required   \
                                           */
#define SLAVE_REQ_RDB_CHANNEL                                                  \
  (1 << 3) /* Use rdb channel replication, transfer RDB background */
/* Mask of all bits in the slave requirements bitfield that represent
 * non-standard (filtered) RDB requirements */
#define SLAVE_REQ_RDB_MASK                                                     \
  (SLAVE_REQ_RDB_EXCLUDE_DATA | SLAVE_REQ_RDB_EXCLUDE_FUNCTIONS |              \
...
#define PAUSE_ACTION_CLIENT_ALL                                                \
  (1 << 1) /* must be bigger than PAUSE_ACTION_CLIENT_WRITE */
...
#define PAUSE_ACTIONS_CLIENT_WRITE_SET                                         \
  (PAUSE_ACTION_CLIENT_WRITE | PAUSE_ACTION_EXPIRE | PAUSE_ACTION_EVICT |      \
   PAUSE_ACTION_REPLICA)
#define PAUSE_ACTIONS_CLIENT_ALL_SET                                           \
  (PAUSE_ACTION_CLIENT_ALL | PAUSE_ACTION_EXPIRE | PAUSE_ACTION_EVICT |        \
   PAUSE_ACTION_REPLICA)
...
#define NOTIFY_NEW                                                             \
  (1 << 14) /* n, new key notification (Note: excluded from NOTIFY_ALL) */
#define NOTIFY_OVERWRITTEN                                                     \
  (1 << 15) /* o, key overwrite notification (Note: excluded from NOTIFY_ALL)  \
             */
#define NOTIFY_TYPE_CHANGED                                                    \
  (1 << 16) /* c, key type changed notification (Note: excluded from           \
               NOTIFY_ALL) */
#define NOTIFY_KEY_TRIMMED                                                     \
  (1 << 17) /* module only key space notification, indicates a key trimmed     \
...
#define serverAssertWithInfo(_c, _o, _e)                                       \
  (likely(_e) ? (void)0                                                        \
              : (_serverAssertWithInfo(_c, _o, #_e, __FILE__, __LINE__),       \
                 redis_unreachable()))
#define serverAssert(_e)                                                       \
  (likely(_e) ? (void)0                                                        \
              : (_serverAssert(#_e, __FILE__, __LINE__), redis_unreachable()))
#define serverPanic(...)                                                       \
  _serverPanic(__FILE__, __LINE__, __VA_ARGS__), redis_unreachable()

/* The following macros provide assertions that are only executed during test
 * builds and should be used to add assertions that are too computationally
 * expensive or dangerous to run during normal operations.  */
...
#define LATENCY_HISTOGRAM_PRECISION                                            \
  2 /* Maintain a value precision of 2 significant digits across               \
     * LATENCY_HISTOGRAM_MIN_VALUE and LATENCY_HISTOGRAM_MAX_VALUE range.      \
     * Value quantization within the range will thus be no larger than 1/100th \
     * (or 1%) of any value. The total size per histogram should sit around 40                             \
     * KiB Bytes. */
...
typedef size_t (*moduleTypeFreeEffortFunc)(struct redisObject *key,
                                           const void *value);
typedef void (*moduleTypeUnlinkFunc)(struct redisObject *key, void *value);
typedef void *(*moduleTypeCopyFunc)(struct redisObject *fromkey,
                                    struct redisObject *tokey,
                                    const void *value);
typedef int (*moduleTypeDefragFunc)(struct RedisModuleDefragCtx *ctx,
                                    struct redisObject *key, void **value);
typedef size_t (*moduleTypeMemUsageFunc2)(struct RedisModuleKeyOptCtx *ctx,
                                          const void *value,
                                          size_t sample_size);
typedef void (*moduleTypeFreeFunc2)(struct RedisModuleKeyOptCtx *ctx,
                                    void *value);
typedef size_t (*moduleTypeFreeEffortFunc2)(struct RedisModuleKeyOptCtx *ctx,
                                            const void *value);
typedef void (*moduleTypeUnlinkFunc2)(struct RedisModuleKeyOptCtx *ctx,
                                      void *value);
typedef void *(*moduleTypeCopyFunc2)(struct RedisModuleKeyOptCtx *ctx,
                                     const void *value);
typedef int (*moduleTypeAuthCallback)(struct RedisModuleCtx *ctx,
...
/* Describe the state of the module during loading, and the indication which
 * configs were loaded / applied already. */
typedef enum {
...
struct RedisModule {
  ...
  ModuleConfigsApplied configs_initialized; /* Have the module configurations
                                               been initialized? */
  ...
  RedisModuleDefragFunc2
      defrag_cb_2; /* Version 2 callback for global data defrag. */
  RedisModuleDefragFunc
      defrag_start_cb;                 /* Callback indicating defrag started. */
  RedisModuleDefragFunc defrag_end_cb; /* Callback indicating defrag ended. */
  struct moduleLoadQueueEntry
      *loadmod; /* Module load arguments for config rewrite. */
  int num_commands_with_acl_categories; /* Number of commands in this module
                                           included in acl categories */
  int onload; /* Flag to identify if the call is being made from Onload (0 or 1)
               */
  size_t num_acl_categories_added; /* Number of acl categories added by this
                                      module. */
};
...
struct RedisModuleDefragCtx {
  ...
  struct redisObject
      *key; /* Optional name of key processed, NULL when unknown. */
...
#define INIT_MODULE_DEFRAG_CTX(endtime, cursor, key, dbid)                     \
  ((RedisModuleDefragCtx){(endtime), (cursor), (key), (dbid),                  \
...
struct RedisModuleIO {
  ...
  int dbid; /* The dbid of the key being processed, -1 when unknown. */
  sds pre_flush_buffer; /* A buffer that should be flushed before next write
                         * operation See rdbSaveSingleModuleAux for more details
                         */
};
...
#define OBJ_ENCODING_ZIPLIST                                                   \
  5                           /* No longer used: old list/hash/zset encoding. */
...
#define OBJ_ENCODING_LISTPACK_EX                                               \
  12 /* Encoded as listpack, extended with metadata */
...
#define OBJ_SHARED_REFCOUNT                                                    \
  ((1 << OBJ_REFCOUNT_BITS) - 1) /* Global object never destroyed. */
#define OBJ_STATIC_REFCOUNT                                                    \
  ((1 << OBJ_REFCOUNT_BITS) - 2) /* Object allocated in the stack. */
...
typedef struct redisDb {
  kvstore *keys;      /* The keyspace for this DB. As metadata, holds keysizes
                         histogram */
  kvstore *expires;   /* Timeout of keys with a timeout set */
  estore *subexpires; /* Timeout of sub-keys with a timeout set. (Currently only
                         used for hashes) */
  dict *blocking_keys; /* Keys with clients waiting for data (BLPOP)*/
  dict *blocking_keys_unblock_on_nokey; /* Keys with clients waiting for
                                         * data, and should be unblocked if key
                                         * is deleted (XREADEDGROUP). This is a
                                         * subset of blocking_keys*/
  dict *stream_claim_pending_keys; /* Keys with clients waiting to claim pending
                                      entries */
...
typedef struct blockingState {
  ...
  int unblock_on_nokey; /* Whether to unblock the client when at least one of
                           the keys is deleted or does not exist anymore */
  /* BLOCKED_LIST, BLOCKED_ZSET and BLOCKED_STREAM or any other Keys related
   * blocking */
 ...
 *
 * After the execution of every command or script, we iterate over this list to
 * check if as a result we should serve data to clients blocked, unblocking
 * them. Note that server.ready_keys will not have duplicates as there
 * dictionary also called ready_keys in every structure representing a Redis
 * database, where we make sure to remember if a given key was already added in
 * the server.ready_keys list. */
 ...
 * replication buffers. Since the replication buffer block list may be very
 * long, it would cost much time to search replication offset on partial resync,
 * so we use one rax tree to index some blocks every
 * REPL_BACKLOG_INDEX_PER_BLOCKS to make searching offset from replication
 * buffer blocks list faster. */
...
#define SHOULD_CLUSTER_COMPATIBILITY_SAMPLE()                                  \
  (server.cluster_compatibility_sample_ratio == 100 ||                         \
   (double)rand() / RAND_MAX * 100 <                                           \
       server.cluster_compatibility_sample_ratio)
...
typedef struct {
  ...
  struct {
    /* General */
    int saved; /* 1 if we already saved the offset (first time we call
                  addReply*) */
...
typedef struct client {
  ...
  uint8_t io_flags;    /* Accessed by both main and IO threads, but not modified
                          concurrently */
  ...
  int original_argc; /* Num of arguments of original command if arguments were
                        rewritten. */
  robj **original_argv;    /* Arguments of original command if arguments were
                              rewritten. */
  size_t all_argv_len_sum; /* Sum of lengths of objects in all pendingCommand
                              argv lists */
  ...
  struct redisCommand *
      realcmd;      /* The original command that was executed by the client,
  ...
  long duration; /* Current command duration. Used for measuring latency of
                    blocking/non-blocking cmds */
  int slot; /* The slot the client is executing against. Set to -1 if no slot is
               being used */
  int cluster_compatibility_check_slot; /* The slot the client is executing
                                         * against for cluster compatibility
                                         * check. -2 means we don't need to
                                         * check slot violation, or we already
                                         * found a violation, reported it and
                                         * don't need to continue checking. -1
                                         * means we're looking for the slot
                                         * number and didn't find it yet. any
                                         * positive number means we found a slot
                                         * and no violation yet. */
  dictEntry *cur_script;  /* Cached pointer to the dictEntry of the script being
                             executed. */
  ...
  long long reploff_next; /* Next value to set for reploff when a command
                             finishes executing */
  long long repl_applied; /* Applied replication data count in querybuf, if this
                             is a replica. */
  long long repl_ack_off; /* Replication ack offset, if this is a slave. */
  long long
      repl_aof_off; /* Replication AOF fsync ack offset, if this is a slave. */
  long long repl_ack_time; /* Replication ack time, if this is a slave. */
  long long
      repl_last_partial_write; /* The last time the server did a partial write
                                  from the RDB child pipe to this replica  */
  ...
  dict *pubsubshard_channels; /* shard level channels a client is interested in
                                 (SSUBSCRIBE) */
  ...
  void *module_blocked_client;   /* Pointer to the RedisModuleBlockedClient
                                  * associated with this   client. This is set in
                                  * case of module authentication before the
                                  * unblocked client is reprocessed to handle
                                  * reply callbacks. */
  void *module_auth_ctx; /* Ongoing / attempted module based auth callback's
                          * ctx. This is only tracked within the context of the
                          * command attempting authentication. If not NULL, it
                          * means module auth is in progress. */
...
#endif
  unsigned long long
      net_input_bytes; /* Total network input bytes read from this client. */
  unsigned long long
      net_output_bytes; /* Total network output bytes sent to this client. */
  unsigned long long
      commands_processed; /* Total count of commands this client executed. */
...
typedef struct __attribute__((aligned(CACHE_LINE_SIZE))) {
  ...
  redisAtomic int
      running;     /* Running if true, main thread can send clients directly. */
  ...
  eventNotifier *pending_clients_notifier; /* Used to wake up the loop when
                                              write should be performed. */
  pthread_mutex_t pending_clients_mutex;   /* Mutex for pending write list */
  list *pending_clients_to_main_thread;    /* Clients that are waiting to be
...
typedef struct aclInfo {
  ...
  long long invalid_cmd_accesses; /* Invalid command accesses that user doesn't
                                     have permission to */
  long long invalid_key_accesses; /* Invalid key accesses that user doesn't have
                                     permission to */
  long long invalid_channel_accesses; /* Invalid channel accesses that user
...
struct sharedObjectsStruct {
  robj *ok, *err, *emptybulk, *czero, *cone, *pong, *space, *queued, *null[4],
      *nullarray[4], *emptymap[4], *emptyset[4], *emptyarray, *wrongtypeerr,
      *nokeyerr, *syntaxerr, *sameobjecterr, *outofrangeerr, *noscripterr,
      *loadingerr, *slowevalerr, *slowscripterr, *slowmoduleerr, *bgsaveerr,
      *masterdownerr, *roslaveerr, *execaborterr, *noautherr, *noreplicaserr,
      *busykeyerr, *oomerr, *plus, *messagebulk, *pmessagebulk, *subscribebulk,
      *unsubscribebulk, *psubscribebulk, *punsubscribebulk, *del, *unlink,
      *rpop, *lpop, *lpush, *rpoplpush, *lmove, *blmove, *zpopmin, *zpopmax,
      *emptyscan, *multi, *exec, *left, *right, *hset, *srem, *xgroup, *xclaim,
      *script, *replconf, *eval, *persist, *set, *pexpireat, *pexpire, *hdel,
      *hpexpireat, *hpersist, *hsetex, *time, *pxat, *absttl, *retrycount,
      *force, *justid, *entriesread, *lastid, *ping, *setid, *keepttl, *load,
      *createconsumer, *fields, *getack, *special_asterick, *special_equals,
      *default_username, *redacted, *ssubscribebulk, *sunsubscribebulk,
      *smessagebulk, *select[PROTO_SHARED_SELECT_CMDS],
 ...
 *
 * int redisOpArrayAppend(redisOpArray *oa, int dbid, robj **argv, int argc, int
 * target); void redisOpArrayFree(redisOpArray *oa);
 */
...
typedef struct redisTLSContextConfig {
  ...
  char *client_cert_file;     /* Certificate to use as a client; if none, use
                                 cert_file */
...
typedef struct {
  aofInfo *
      base_aof_info; /* BASE file information. NULL if there is no BASE file. */
  list *incr_aof_list;    /* INCR AOFs list. We may have multiple INCR AOF when
                             rewrite fails. */
  list *history_aof_list; /* HISTORY AOF list. When the AOFRW success, The
                             aofInfo contained in `base_aof_info` and
                             `incr_aof_list` will be moved to this list. We will
                             delete these AOF files when AOFRW finish. */
  long long curr_base_file_seq; /* The sequence number used by the current BASE
                                   file. */
  long long curr_incr_file_seq; /* The sequence number used by the current INCR
                                   file. */
  int dirty; /* 1 Indicates that the aofManifest in the memory is inconsistent
                with disk, we need to persist it immediately. */
} aofManifest;
...
struct redisServer_size_check {
  ...
  redisAtomic long long
      stat_net_repl_input_bytes; /* Bytes read during replication, added to
                                    stat_net_input_bytes in 'info'. */
  redisAtomic long long
  ...
  kvstore *pubsubshard_channels; /* Map shard channels in every slot to list of
                                    subscribed clients */
...
struct redisServer {
  ...
  int errors_enabled; /* If true, errorstats is enabled, and we will add new
                         errors. */
  ...
  redisAtomic int last_sig_received; /* Indicates the last SIGNAL received, if
                                        any (e.g., SIGINT or SIGTERM). */
  ...
  int busy_module_yield_flags; /* Are we inside a busy module? (triggered by
                                  RM_Yield). see BUSY_MODULE_YIELD_ flags. */
  const char
      *busy_module_yield_reply; /* When non-null, we are inside RM_Yield. */
  char *ignore_warnings;        /* Config: warnings that should be ignored. */
  int client_pause_in_transaction; /* Was a client pause executed during this
                                      Exec? */
  int thp_enabled;                 /* If true, THP is enabled. */
  size_t page_size;                /* The page size of OS. */
  redisAtomic int running; /* Running if true, IO threads can send clients
                              without notification */
  /* Modules */
  dict *moduleapi;            /* Exported core APIs dictionary for modules. */
  dict *sharedapi;            /* Like moduleapi but containing the APIs that
                                 modules share with each other. */
  dict *module_configs_queue; /* Unmapped configs are queued here, assumed to be
                                 module config. Applied after modules are loaded
                                 during startup or arguments to loadex. */
  list *loadmodule_queue;     /* List of modules to load at startup. */
  int module_pipe[2]; /* Pipe used to awake the event loop by module threads. */
  pid_t child_pid;    /* PID of current child */
  redisAtomic int module_gil_acquring; /* Indicates whether the GIL is being
                                          acquiring by the main thread. */
  /* Networking */
  int tcp_backlog;                     /* TCP listen() backlog */
  char *bindaddr[CONFIG_BINDADDR_MAX]; /* Addresses we should bind to */
  int bindaddr_count; /* Number of addresses in server.bindaddr[] */
  char *
      bind_source_addr; /* Source address to bind on for outgoing connections */
  ...
  client *current_client;   /* The client that triggered the command execution
                               (External or AOF). */
  client *executing_client; /* The client executing the current command
                               (possibly script or module). */

#ifdef LOG_REQ_RES
  char *req_res_logfile; /* Path of log file for logging all requests and their
                            replies. If NULL, no logging will be performed */
  ...
  int execution_nesting;   /* Execution nesting level.
                            * e.g. call(), async module stuff (timers, events,
                            * etc.),   cron stuff (active expire, eviction) */
  ...
  int io_threads_clients_num[IO_THREADS_MAX_NUM]; /* Number of clients assigned
                                                     to each IO thread. */
  int io_threads_do_reads;     /* Read and parse from IO threads? */
  pendingCommandPool cmd_pool; /* Shared pool for reusing pendingCommand,
                                * only when IO threads disabled */
  int prefetch_batch_max_size; /* Maximum number of keys to prefetch in a single
                                  batch */
  long long events_processed_while_blocked; /* processEventsWhileBlocked() */
  int enable_protected_configs; /* Enable the modification of protected configs,
                                   see PROTECTED_ACTION_ALLOWED_* */
  int enable_debug_cmd; /* Enable DEBUG commands, see PROTECTED_ACTION_ALLOWED_*
                         */
  int enable_module_cmd; /* Enable MODULE commands, see
                            PROTECTED_ACTION_ALLOWED_* */
  ...
  long long stat_cluster_incompatible_ops; /* Number of operations that are
                                              incompatible with cluster mode */
  /* The following two are used to track instantaneous metrics, like
   * number of operations per second, network traffic. */
  monotime el_start;
  /* The following two are used to record the max number of commands executed in
   * one eventloop. Note that commands in transactions are also counted. */
  long long el_cmd_cnt_start;
  long long el_cmd_cnt_max;
  /* The sum of active-expire, active-defrag and all other tasks done by cron
     and beforeSleep, but excluding read, write and AOF, which are counted by
     other sets of metrics. */
  monotime el_cron_duration;

  /* Configuration */
  int verbosity;               /* Loglevel in redis.conf */
  int hide_user_data_from_log; /* In the event of an assertion failure, hide
                                  command arguments from the operator */
  ...
  int sanitize_dump_payload;    /* Enables deep sanitization for ziplist and
                                   listpack in RDB and RESTORE. */
  int skip_checksum_validation; /* Disable checksum validation for RDB and
                                   RESTORE payload. */
  int jemalloc_bg_thread;       /* Enable jemalloc background thread */
  int active_defrag_configuration_changed; /* defrag configuration has been
                                            * changed and need to reconsider
                                            * active_defrag_running in
                                            * computeDefragCycles. */
  ...
  int lookahead;  /* how many commands in each client pipeline to decode and
                     prefetch */
  ...
  int dict_resizing; /* Whether to allow main dict and expired dict to be
                        resized (debug) */
  int latency_tracking_enabled; /* 1 if extended latency tracking is enabled, 0
                                   otherwise. */
  double *latency_tracking_info_percentiles; /* Extended latency tracking info
                                                output percentile list
                                                configuration. */
  int latency_tracking_info_percentiles_len;
  int memory_tracking_per_slot; /* Account used memory per slot */
  unsigned int
      max_new_tls_conns_per_cycle; /* The maximum number of tls connections that
                                      will be accepted during each invocation of
                                      the event loop. */
  unsigned int max_new_conns_per_cycle; /* The maximum number of tcp connections
                                           that will be accepted during each
                                           invocation of the event loop. */
  int cluster_compatibility_sample_ratio; /* Sampling ratio for cluster mode
                                             incompatible commands. */
  int lazyexpire_nested_arbitrary_keys;   /* If disabled, avoid lazy-expire from
                                             commands that touch arbitrary keys
                                             (SCAN/RANDOMKEY) within transactions
                                           */
  ...
  off_t aof_last_incr_fsync_offset; /* AOF offset which is already requested to
                                     * be synced to disk. Compare with the
                                     * aof_last_incr_size. */
  ...
  off_t aof_load_corrupt_tail_max_size; /* The max size of broken AOF tail than
                                           can be ignored. */
  int aof_use_rdb_preamble; /* Specify base AOF to use RDB encoding on AOF
                               rewrites. */
  redisAtomic int aof_bio_fsync_errno; /* Errno of AOF fsync in bio job. */
  aofManifest *aof_manifest;           /* Used to track AOFs. */
  int aof_disable_auto_gc; /* If disable automatically deleting HISTORY type
                              AOFs? default no. (for testings). */

  /* RDB persistence */
  long long dirty_before_bgsave; /* Used to restore dirty on failed BGSAVE */
  int bgsave_aborted; /* Set when killing a child, to treat it as aborted even
                         if it succeeds. */
  ...
  /* Replication (master) */
  redisAtomic long long
      fsynced_reploff_pending; /* Largest replication offset to
                  * potentially have been fsynced, applied to
                    fsynced_reploff only when AOF state is AOF_ON
                    (not during the initial rewrite) */
  long long fsynced_reploff;   /* Largest replication offset that has been
                                  confirmed to be fsynced */
  int slaveseldb;              /* Last SELECTed DB in replication output */
  int repl_ping_slave_period;  /* Master pings the slave every N seconds */
  replBacklog *repl_backlog;   /* Replication backlog for partial syncs */
  long long repl_full_sync_buffer_limit; /* Accumulated repl data limit during
  ...
  int repl_diskless_sync_max_replicas; /* Max replicas for diskless repl BGSAVE
                                        * delay (start sooner if they all
                                        * connect). */
  ...
  time_t repl_stream_lastio; /* Unix time of the latest sending replication
                                stream. */
  /* Replication (slave) */
  ...
  int repl_rdb_ch_state;   /* State of the replica's rdb channel during rdb
                              channel replication */
  int repl_main_ch_state;  /* State of the replica's main channel during rdb
                              channel replication */
  uint64_t repl_num_master_disconnection; /* Number of master connection was
                                             disconnected */
  uint64_t repl_main_ch_client_id;        /* Main channel client id received in
                                             +RDBCHANNELSYNC reply. */
  off_t repl_transfer_size; /* Size of RDB to read from master during sync. */
  off_t repl_transfer_read; /* Amount of RDB read from master during sync. */
  off_t repl_transfer_last_fsync_off; /* Offset when we fsync-ed last time. */
  connection *repl_transfer_s;        /* Slave -> Master SYNC connection */
  connection *repl_rdb_transfer_s; /* Slave -> Master FULL SYNC connection (RDB
                                      download) */
  ...
  int propagation_error_behavior;   /* Configures the behavior of the replica
                                     * when it receives an error on the
                                     * replication stream */
  int repl_ignore_disk_write_error; /* Configures whether replicas panic when
                                     * unable to persist writes to AOF. */
  ...
  long long repl_current_sync_attempts; /* Number of times in current
                                           configuration, the replica attempted
                                           to sync since the last success. */
  long long
      repl_total_sync_attempts; /* Number of times in current configuration, the
                                   replica attempted to sync to a master  */
  time_t repl_disconnect_start_time; /* Unix time that master disconnection
                                        start */
  time_t repl_total_disconnect_time; /* The total cumulative time we've been
                                        disconnected as a replica, visible when
                                        the link is up too. */
  /* Limits */
  ...
  list *pending_push_messages; /* pending publish or other push messages to
                                  flush */
  ...
  size_t blocking_op_nesting; /* Nesting level of blocking operation, used to
                                 reset blocked_last_cron. */
  long long blocked_last_cron; /* Indicate the mstime of the last time we did
                                  cron jobs from a blocking operation */
  /* Pubsub */
  ...
  mstime_t cluster_ping_interval; /* A debug configuration for setting how often
                                     cluster nodes send ping messages. */
  char *cluster_configfile;       /* Cluster auto-generated config file name. */
  long long asm_handoff_max_lag_bytes; /* Maximum lag in bytes before pausing
                                          writes for ASM handoff. */
  long long asm_write_pause_timeout; /* Timeout in milliseconds to pause writes
                                        during ASM handoff. */
  long long asm_sync_buffer_drain_timeout; /* Timeout in milliseconds for sync
                                              buffer to drain during ASM. */
  int asm_max_archived_tasks; /* Maximum number of archived ASM tasks to keep in
                                 memory. */
  struct clusterState *cluster; /* State of the cluster */
  struct clusterSlotStat
      *cluster_slot_stats; /* Struct used for storing slot statistics, for all
                              slots owned by the current shard. */
  int cluster_migration_barrier;       /* Cluster replicas migration barrier. */
  int cluster_allow_replica_migration; /* Automatic replica migrations to
                                          orphaned masters and from empty
                                          masters */
  ...
  char *cluster_announce_human_nodename; /* Human readable node name assigned to
                                            a node. */
  int cluster_preferred_endpoint_type;   /* Use the announced hostname when
                                            available. */
  ...
  unsigned long long
      cluster_link_msg_queue_limit_bytes; /* Memory usage limit on individual
                                             link msg queue */
  int cluster_drop_packet_filter;         /* Debug config that allows tactically
                                           * dropping packets of a specific type */
  int cluster_slot_stats_enabled; /* Cluster slot usage statistics tracking
                                     enabled. */
  /* Scripting */
  unsigned int lua_arena;         /* eval lua arena used in jemalloc. */
  mstime_t busy_reply_threshold;  /* Script / module timeout in milliseconds */
  int pre_command_oom_state;      /* OOM before command (script?) was started */
  int script_disable_deny_script; /* Allow running commands marked "noscript"
                                     inside a script. */
  ...
  /* Sentinel config */
  struct sentinelConfig
      *sentinel_config; /* sentinel config to load at startup time. */
  ...
  int cluster_allow_pubsubshard_when_down; /* Is pubsubshard allowed when the
                                              cluster is down, doesn't affect
                                              pubsub global. */
  long reply_buffer_peak_reset_time; /* The amount of time (in milliseconds) to
                                        wait between reply buffer peak resets */
  int reply_buffer_resizing_enabled; /* Is reply buffer resizing enabled (1 by
                                        default) */
  /* Local environment */
  char *locale_collate;
  int dbg_assert_keysizes; /* Assert keysizes histogram after each command */
  int dbg_assert_alloc_per_slot; /* Assert per-slot alloc_size after each
                                    command */
};
...
typedef struct {
  ...
  keyReference keysbuf[MAX_KEYS_BUFFER]; /* Pre-allocated buffer, to save heap
                                            allocations */
...
enum {
  PENDING_CMD_FLAG_INCOMPLETE =
      1 << 0, /* Command parsing is incomplete, still waiting for more data */
  PENDING_CMD_FLAG_PREPROCESSED =
      1 << 1, /* This command has passed pre-processing */
  PENDING_CMD_KEYS_RESULT_VALID =
      1 << 2, /* Command's keys_result is valid and cached */
};
...
struct pendingCommand {
  ...
  long long reploff; /* c->reploff should be set to this value when the command
                        is processed */
  int flags;
  int slot; /* The slot the command is executing against. Set to
             * INVALID_CLUSTER_SLOT if no slot is being used or if the command
             * has a cross slot error */
  uint8_t read_error;
 ...
 * 2. find_keys (FK): relative to the output of BS, how can we will which args
 * are keys?
 *
 ...
 * 2. keynum: there's an arg that contains the number of key args somewhere
 * before the keys themselves
 */

/* WARNING! Must be synced with generate-command-code.py and
...
typedef struct {
  ...
  union {
    /* NOTE: Indices in this struct are relative to the result of the
     * begin_search step! These are: range.lastkey, keynum.keynumidx,
     * keynum.firstkey */
    struct {
      /* Index of the last key.
       * Can be negative, in which case it's not relative. -1 indicating till
       * the last argument, -2 one before the last and so on. */
      int lastkey;
      /* How many args should we skip after finding a key, in order to find the
       * next one. */
      int keystep;
      /* If lastkey is -1, we use limit to stop the search by a factor. 0 and 1
       * mean no limit. 2 means 1/2 of the remaining args, 3 means 1/3, and so
       * on. */
      int limit;
    } range;
    struct {
 ...
 *
 * CMD_READONLY:    Commands just reading from keys without changing the
 * content. Note that commands that don't read from the keyspace such as TIME,
 * SELECT, INFO, administrative commands, and connection or transaction related
 * commands (multi, exec, discard, ...) are not flagged as read-only commands,
 * since they affect the server or the connection in other ways.
 *
 ...
 *                  command will be accepted in cluster mode if the slot is
 * marked as 'importing'.
 *
 * CMD_FAST:        Fast command: O(1) or O(log(N)) command that should never
 *                  delay its execution as long as the kernel scheduler is
 * giving us time. Note that commands that may trigger a DEL as a side effect
 * (like SET) are not fast commands.
 *
 ...
 *                      allowed under circumstances where write commands are
 * disallowed. Examples include PUBLISH, which replicates pubsub messages,and
 *                      EVAL, which may execute write commands, which are
 * replicated, or may just execute read commands. A command can not be marked
...
struct redisCommand {
  ...
  const char *declared_name; /* A string representing the command declared_name.
                              * It is a const char * for native commands and SDS
                              * for module commands. */
  const char *summary;       /* Summary of the command (optional). */
  const char *complexity;    /* Complexity description (optional). */
  const char *since;         /* Debut version of the command (optional). */
  int doc_flags;             /* Flags for documentation (see CMD_DOC_*). */
  const char *replaced_by;   /* In case the command is deprecated, this is the
                                successor command. */
  const char *deprecated_since; /* In case the command is deprecated, when did
                                   it happen? */
  redisCommandGroup group;      /* Command group */
  commandHistory *history;      /* History of the command */
  int num_history;
  const char **tips; /* An array of strings that are meant to be tips for
                        clients/proxies regarding this command */
  int num_tips;
  ...
  struct hdr_histogram
      *latency_histogram; /*points to the command latency command histogram
                             (unit of time nanosecond) */
  keySpec legacy_range_key_spec; /* The legacy (first,last,step) key spec is
                                  * still maintained (if applicable) so that
                                  * we can still support the reply format of
                                  * COMMAND INFO and COMMAND GETKEYS */
  dict *subcommands_dict; /* A dictionary that holds the subcommands, the key is
                           * the subcommand sds name (not the fullname), and the
                           * value is the redisCommand structure pointer. */
  struct redisCommand *parent;
  struct RedisModuleCommand *module_cmd; /* A pointer to the module command data
                                            (NULL if native command) */
};
...
int moduleOnLoad(int (*onload)(void *, void **, int), const char *path,
                 void *handle, void **module_argv, int module_argc,
                 int is_loadex);
...
int moduleGetCommandKeysViaAPI(struct redisCommand *cmd, robj **argv, int argc,
                               getKeysResult *result);
int moduleGetCommandChannelsViaAPI(struct redisCommand *cmd, robj **argv,
                                   int argc, getKeysResult *result);
...
sds modulesCollectInfo(sds info, dict *sections_dict, int for_crash_report,
                       int sections);
...
robj *moduleTypeDupOrReply(client *c, robj *fromkey, robj *tokey, int todb,
                           robj *value);
int moduleDefragValue(robj *key, robj *obj, int dbid);
int moduleLateDefrag(robj *key, robj *value, unsigned long *cursor,
                     monotime endtime, int dbid);
...
void addExtendedReplyHelp(client *c, const char **help,
...
void pauseActions(pause_purpose purpose, mstime_t end,
                  uint32_t actions_bitmask);
...
/* reply macros */
#define ADD_REPLY_BULK_CBUFFER_STRING_CONSTANT(c, str)                         \
  addReplyBulkCBuffer(c, str, strlen(str))
...
void listTypeSetIteratorDirection(listTypeIterator *li, listTypeEntry *entry,
                                  unsigned char direction);
int listTypeNext(listTypeIterator *li, listTypeEntry *entry);
robj *listTypeGet(listTypeEntry *entry);
unsigned char *listTypeGetValue(listTypeEntry *entry, size_t *vlen,
                                long long *lval);
...
void listElementsRemoved(client *c, robj *key, int where, robj *o, long count,
                         size_t oldsize, int signal, int *deleted);
typedef enum {
  LIST_CONV_AUTO,
  LIST_CONV_GROWING,
  LIST_CONV_SHRINKING,
...
void listTypeTryConversion(robj *o, list_conv_type lct, beforeConvertCB fn,
                           void *data);
...
int getPositiveLongFromObjectOrReply(client *c, robj *o, long *target,
                                     const char *msg);
int getRangeLongFromObjectOrReply(client *c, robj *o, long min, long max,
                                  long *target, const char *msg);
...
void replDataBufReadFromConn(connection *conn, replDataBuf *buf,
...
void sendChildInfoGeneric(childInfoType info_type, size_t keys, double progress,
                          char *pname);
...
int checkModuleAuthentication(client *c, robj *username, robj *password,
                              robj **err);
...
int ACLCheckAllUserCommandPerm(user *u, struct redisCommand *cmd, robj **argv,
                               int argc, getKeysResult *key_result,
                               int *idxptr);
...
void addACLLogEntry(client *c, int reason, int context, int argpos,
                    sds username, sds object);
sds getAclErrorMessage(int acl_res, user *user, struct redisCommand *cmd,
                       sds errored_val, int verbose);
...
#define ZADD_IN_GT                                                             \
  (1 << 3) /* Only update existing when new scores are higher. */
#define ZADD_IN_LT                                                             \
  (1 << 4) /* Only update existing when new scores are lower. */

/* Output flags. */
#define ZADD_OUT_NOP                                                           \
  (1 << 0) /* Operation not performed because of conditionals.*/
#define ZADD_OUT_NAN (1 << 1)   /* Only touch elements already existing. */
#define ZADD_OUT_ADDED (1 << 2) /* The element was new and was added. */
#define ZADD_OUT_UPDATED                                                       \
  (1 << 3) /* The element already existed, score updated. */
...
#define ERROR_COMMAND_REJECTED                                                 \
  (1 << 0) /* Indicate to update the command rejected stats */
#define ERROR_COMMAND_FAILED                                                   \
  (1 << 1) /* Indicate to update the command failed stats */
...
void genericZpopCommand(client *c, robj **keyv, int keyc, int where,
                        int emitkey, long count, int use_nested_array,
...
struct redisCommand *lookupSubcommand(struct redisCommand *container,
                                      sds sub_name);
...
int redisOpArrayAppend(redisOpArray *oa, int dbid, robj **argv, int argc,
                       int target);
...
void slowlogPushCurrentCommand(client *c, struct redisCommand *cmd,
                               ustime_t duration);
void updateCommandLatencyHistogram(struct hdr_histogram **latency_histogram,
                                   int64_t duration_hist);
...
unsigned long long dbScan(redisDb *db, unsigned long long cursor,
                          dictScanFunction *scan_cb, void *privdata);
...
int setTypeConvertAndExpand(robj *setobj, int enc, unsigned long cap,
                            int panic);
...
typedef struct htMetadataEx {
  ...
  ExpireMeta expireMeta; /* embedded ExpireMeta in dict.
                            ...
                            subexpires DB with next minimum hash-field to
                            expire. TTL value might be inaccurate up-to few
                            seconds due to optimization consideration. */
...
/* Hash field lazy expiration flags. Used by core hashTypeGetValue() and its
 * callers */
#define HFE_LAZY_EXPIRE                                                        \
  (0) /* Delete expired field, and if last field also the hash */
#define HFE_LAZY_AVOID_FIELD_DEL (1 << 0) /* Avoid deleting expired field */
#define HFE_LAZY_AVOID_HASH_DEL                                                \
  (1 << 1) /* Avoid deleting hash if the field is the last one */
#define HFE_LAZY_NO_NOTIFICATION                                               \
  (1 << 2) /* Do not send notification, used when multiple fields              \
            * may expire and only one notification is desired. */
#define HFE_LAZY_NO_SIGNAL                                                     \
  (1 << 3) /* Do not send signal, used when multiple fields                    \
            * may expire and only one signal is desired. */
#define HFE_LAZY_ACCESS_EXPIRED                                                \
  (1 << 4) /* Avoid lazy expire and allow access to expired fields */
#define HFE_LAZY_NO_UPDATE_KEYSIZES                                            \
  (1 << 5) /* If field lazy deleted, avoid updating keysizes histogram */
#define HFE_LAZY_NO_UPDATE_ALLOCSIZES                                          \
  (1 << 6) /* If field lazy deleted, avoid updating slot allocation sizes */

void hashTypeConvert(redisDb *db, robj *o, int enc);
void hashTypeTryConversion(redisDb *db, kvobj *kv, robj **argv, int start,
                           int end);
int hashTypeExists(redisDb *db, kvobj *kv, sds field, int hfeFlags,
                   int *isHashDeleted);
...
void hashTypeCurrentFromListpack(hashTypeIterator *hi, int what,
                                 ...
                                 long long *vll, uint64_t *expireTime);
...
void listpackExAddNew(robj *o, char *field, size_t flen, char *value,
                      size_t vlen, uint64_t expireAt);
...
static inline void hfieldFree(hfield field, size_t *usable) {
  mstrFree(&mstrFieldKind, field, usable);
}
static inline void *hfieldGetAllocPtr(hfield field) {
  return mstrGetAllocPtr(&mstrFieldKind, field);
}
...
#define IMMUTABLE_CONFIG                                                       \
  (1ULL << 0) /* Can this value only be set at startup? */
#define SENSITIVE_CONFIG                                                       \
  (1ULL << 1) /* Does this value contain sensitive information */
#define DEBUG_CONFIG (1ULL << 2) /* Values that are useful for debugging. */
#define MULTI_ARG_CONFIG                                                       \
  (1ULL << 3) /* This config receives multiple arguments. */
#define HIDDEN_CONFIG                                                          \
  (1ULL << 4) /* This config is hidden in `config get <pattern>` (used for     \
                 tests/debugging) */
#define PROTECTED_CONFIG                                                       \
  (1ULL << 5) /* Becomes immutable if enable-protected-configs is enabled. */
#define DENY_LOADING_CONFIG                                                    \
  (1ULL << 6) /* This config is forbidden during loading. */
#define ALIAS_CONFIG                                                           \
  (1ULL                                                                        \
   << 7) /* For configs with multiple names, this flag is set on the alias. */
#define MODULE_CONFIG (1ULL << 8) /* This config is a module config */
#define VOLATILE_CONFIG                                                        \
  (1ULL                                                                        \
   << 9) /* The config is a reference to the config data and not the config    \
          * data itself (ex. a file name containing more configuration like a                              \
          * tls key). In this case we want to apply the configuration change                                             \
          * even if the new config value is the same as the old. */

#define INTEGER_CONFIG 0 /* No flags means a simple integer configuration */
#define MEMORY_CONFIG                                                          \
  (1 << 0) /* Indicates if this value can be loaded as a memory value */
#define PERCENT_CONFIG                                                         \
  (1 << 1) /* Indicates if this value can be loaded as a percent (and stored   \
              as a negative int) */
#define OCTAL_CONFIG (1 << 2) /* This value uses octal representation */

/* Enum Configs contain an array of configEnum objects that match a string with
 * an integer. */
...
int rewriteConfigRewriteLine(struct rewriteConfigState *state,
...
static inline int clusterSlotStatsEnabled(void) {
...
void addModuleBoolConfig(sds name, sds alias, int flags, void *privdata,
                         int default_val);
void addModuleStringConfig(sds name, sds alias, int flags, void *privdata,
                           sds default_val);
void addModuleEnumConfig(sds name, sds alias, int flags, void *privdata,
                         int default_val, configEnum *enum_vals,
                         int num_enum_vals);
void addModuleNumericConfig(sds name, sds alias, int flags, void *privdata,
                            long long default_val, int conf_flags,
...
int setModuleNumericConfig(ModuleConfig *config, long long val,
                           const char **err);

/* API for modules to access config values. */
dictIterator *moduleGetConfigIterator(void);
const char *moduleConfigIteratorNext(dictIterator **iter, sds pattern,
                                     int is_glob, configType *typehint);
...
int moduleSetStringConfig(client *c, sds name, const char *val,
                          const char **err);
int moduleSetEnumConfig(client *c, sds name, sds *vals, int vals_cnt,
                        const char **err);
int moduleSetNumericConfig(client *c, sds name, long long val,
                           const char **err);

/* db.c -- Keyspace access API */
void updateKeysizesHist(redisDb *db, int didx, uint32_t type, int64_t oldLen,
                        int64_t newLen);
...
void deleteEvictedKeyAndPropagate(redisDb *db, robj *keyobj,
...
kvobj *setExpireByLink(client *c, redisDb *db, sds key, long long when,
                       dictEntryLink link);
...
#define LOOKUP_NONOTIFY                                                        \
  (1 << 1) /* Don't trigger keyspace event on key misses. */
#define LOOKUP_NOSTATS                                                         \
  (1 << 2)                    /* Don't update keyspace hits/misses counters. */
...
#define LOOKUP_NOEFFECTS                                                       \
  (LOOKUP_NONOTIFY | LOOKUP_NOSTATS | LOOKUP_NOTOUCH |                         \
   LOOKUP_NOEXPIRE) /* Avoid any effects from fetching the key */

static inline kvobj *dictGetKV(const dictEntry *de) {
  return (kvobj *)dictGetKey(de);
}
...
void dbReplaceValue(redisDb *db, robj *key, kvobj **ioKeyVal,
                    int updateKeySizes);
void dbReplaceValueWithLink(redisDb *db, robj *key, robj **val,
                            dictEntryLink link);
...
kvobj *dbUnshareStringValueByLink(redisDb *db, robj *key, kvobj *kv,
                                  dictEntryLink link);
...
#define GET_KEYSPEC_INCLUDE_NOT_KEYS (1 << 0) /* Consider 'fake' keys as keys  \
                                               */
#define GET_KEYSPEC_RETURN_PARTIAL                                             \
  (1 << 1) /* Return all keys that can be found */

int getKeysFromCommandWithSpecs(struct redisCommand *cmd, robj **argv, int argc,
                                int search_flags, getKeysResult *result);
...
int sintercardGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                      getKeysResult *result);
int zunionInterDiffGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                           getKeysResult *result);
int zunionInterDiffStoreGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                                getKeysResult *result);
int evalGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                getKeysResult *result);
int functionGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                    getKeysResult *result);
int sortGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                getKeysResult *result);
int sortROGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                  getKeysResult *result);
...
int xreadGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                 getKeysResult *result);
int lmpopGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                 getKeysResult *result);
int blmpopGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                  getKeysResult *result);
int zmpopGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                 getKeysResult *result);
int bzmpopGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                  getKeysResult *result);
int setGetKeys(struct redisCommand *cmd, robj **argv, int argc,
               getKeysResult *result);
int delexGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                 getKeysResult *result);
int bitfieldGetKeys(struct redisCommand *cmd, robj **argv, int argc,
                    getKeysResult *result);
...
void freeLuaScriptsSync(dict *lua_scripts, list *lua_scripts_lru_list,
                        lua_State *lua);
void freeLuaScriptsAsync(dict *lua_scripts, list *lua_scripts_lru_list,
                         lua_State *lua);
...
void blockForReplication(client *c, mstime_t timeout, long long offset,
                         long numreplicas);
void blockForAofFsync(client *c, mstime_t timeout, long long offset,
                      int numlocal, long numreplicas);
...
void totalNumberOfStatefulKeys(unsigned long *blocking_keys,
                               unsigned long *bloking_keys_on_nokey,
...
int dictSdsMstrKeyCompare(dictCmpCache *cache, const void *sdsLookup,
                          const void *mstrStored);
...
dict *genInfoSectionDict(robj **argv, int argc, char **defaults, int *out_all,
                         int *out_everything);
...
void commandAddSubcommand(struct redisCommand *parent,
                          struct redisCommand *subcommand,
